...
import 'package:im_ui/src/util/iterable_ext.dart';
import 'im_indexes_anchor.dart';
import 'im_indexes_list.dart';
import 'sticky_header/sticky_header_widget.dart';

/// 索引
/// 移除了特定的字体、大小、颜色和圆角样式，使用默认值代替
class IMIndexes extends StatefulWidget {
  const IMIndexes({
    super.key,
    this.indexList,
  ...
  @override
  State<IMIndexes> createState() => _IMIndexesState();
}

class _IMIndexesState extends State<IMIndexes> {
  ...
  @override
  void didUpdateWidget(IMIndexes oldWidget) {
  ...
  Widget build(BuildContext context) {
    return Container(
      color: Colors.white, // 原TDTheme.of(context).whiteColor1
      child: Stack(
        children: [
          CustomScrollView(
  ...
  List<Widget> _slivers() {
    ...
    return _indexList.map((e) {
      ...
      return SliverStickyHeader.builder(
        ...
        pinnedOffset: isPinnedOffset ? 12 + stickyOffset : stickyOffset, // 原TDTheme.of(context).spacer8 + stickyOffset
        ...
        sliver: SliverToBoxAdapter(
          child: Builder(
            builder: (context) {
              ...
              return Padding(
                padding: isPinnedOffset ? const EdgeInsets.only(top: 12) : EdgeInsets.zero, // 原EdgeInsets.only(top: TDTheme.of(context).spacer8)
